 ...
 foreach ($vc in $vcs)
 {
  ...
  foreach ($DC in $DCs)
  {
   ...
   Get-Cluster -Location $DC|sort-object name| %{
    ...
    $allvmdetails += $cluster | Get-VM|Sort-Object name|select @{n="vmname";e={$_.name}},@{n="Guest IP";e={$_.guest.ipAddress}}, `
...
$vcenterdetails|export-csv -NoTypeInformation "$location\$vcenters_vmdetails_$date.csv"
